...
// pool合约 + periphery 外围合约
// 1.外围合约从创建池子=>做市商注入流动性铸造NFT头寸=>更新流动性,更新tick
// 2.普通用户交易,外围合约解析调用路径path,然后调用pool合约 swap 函数，最后算出要兑换的tokenout金额
// 过程中涵盖费用,预言机,闪电贷等内容。
// 还有很多基础库,数学库,定点数,汇编等知识点
// 熟悉完 uniswap 的实现代码后，阅读其他类似交易所的代码，就会容易很多
...
contract UniswapV3Pool is IUniswapV3Pool, NoDelegateCall {
    ...
    /// @inheritdoc IUniswapV3PoolImmutables
    // 每个流动性池都划分为多个价格区间（Ticks），该变量定义了每个 Tick 之间的距离。Uniswap V3 通过这种方式实现集中流动性
    ...
    /// @inheritdoc IUniswapV3PoolDerivedState
    // 获取指定过去时间段的价格，以便计算 TWAP（时间权重平均价格）
        ...
        returns (Position.Info storage position, int256 amount0, int256 amount1)
    {
        ...
        // 当前市场价格的tick小于头寸下限tick,那么头寸里月有资金都应该是tokene
    ...
    function swap(
        ...
        if (zeroForOne) {
            ...
            // 最后校验，之前的余额 + swap 转出去的金额 <= 当前的余额（只能多不能少，否则这个 swap 数量就不对了）
